...
-   **購読者 (`BaseSpriteView`, Managerクラスなど):** `GameEventBus.On<T>()` で特定のコマンドを購読し、コールバック関数内で処理を実行します。
...
-   **現状:** モックアップ段階では、一部の ScriptableObject (例: `UIStateMachine`, `GameEventBus`) は、便宜上 `Instance` プロパティによるシングルトンアクセスを提供している場合があります。これは開発の迅速化を目的とした一時的な措置です。
